...
public class main {
	...
	public static void executionJar(String[] args) {
		...
		if (args.length == 1) {
			...
			System.out
					.println("Veuillez rentrer un paramètre (chemin entre \"\")");
			System.out
					.println("Vous avez entré " + args.length + "paramètres:");
	...
	/**
	 * Le début de l'algorithme comme tel. L'ordre de modification est très
	 * logique et il serait difficile de combiner les étapes sans obtenir des
	 * fichiers oubliés.
	 * 
	 * TODO Faire en une étape sans oublier de fichiers et garder l'impact sur
	 * la mémoire bas (gl)
	 * 
	 * @param racine
	 *            La racine à analyser
	 */
	public static void debutAlgo(String racine) {
		...
		if (nbErreurs > 0) {
			System.out
					.println(""
							+ nbErreurs
							+ " ERREURS lors de l'exécution, vérifiez les doublons possibles et les droits d'écriture!");
		}
		System.out.println("Exécuté en " + (System.currentTimeMillis() - temps)
				+ "ms");
			...
			if (r.changement) {
				if (nouveauDos.exists()) {
					System.out.println("Erreur, le dossier "
							+ nouveauDos.getAbsolutePath() + " existe déjà !");
					nbErreursDos++;
				} else {
					nbMods++;
			...
			if (r.changement) {
				if (nouveauFic.exists()) {
					System.out.println("Erreur, le fichier "
							+ nouveauFic.getAbsolutePath() + " existe déjà !");
					nbErreursFilms++;
				} else {
					nbMods++;
	...
	/**
	 * Algorithme récursif qui cherche les fichiers Les fichiers qui respectent
	 * les critères seront dans ficCumulatif
	 * 
	 * @param chemin
	...
	/**
	 * Algorithme qui cherche les films qui sont à la racine sans dossier. Un
	 * dossier avec le nom du film sera créé pour chaque fichier trouvé
	 * 
	 * @param racine
	 *            racine
	 */
	...
	public static void securiserOrphelins(String racine) {
		...
		for (File fic : repRacine.listFiles()) {
			if (fic.isFile() && fic.getName().contains(".")
				...
				if (nouveauFic.exists()) {
					System.out.println("Erreur, le fichier "
							+ dir.getAbsolutePath() + " existe déjà !");
					nbErreursOrp++;
				} else {
					dir.mkdir();
